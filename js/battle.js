...
function makeWaterNormalTex(){
  ...
  const rnd = mulberry32 ? mulberry32(7331) : Math.random;   // deterministic if the seeded RNG is in scope
  for (let i = 0; i < 240; i++){
...
class Sim {
  ...
  buildWorld(){
    ...
    if (env.sea){
      ...
      groundMat = new THREE.MeshStandardMaterial({
        ...
        normalMap: this.seaNormTex, normalScale: new THREE.Vector2(0.55, 0.55),   // survives, but the reflection stops aliasing when adaptive res drops
  ...
  splashDamage(center, radius, dmg, attacker, team, directTarget = null, directMultiplier = 1){
    if (radius <= 0){ return; }
    for (const o of this.craft){
