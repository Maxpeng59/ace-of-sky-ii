...
export const WEAPONS = {
  ...
  ir:          { key: 'ir',          name: 'IR Missile',     type: 'missile',     dmg: 500, rof: 0.5, speed: 600,  clip: 2,    reload: 6, spread: 0,     splash: 24, lockTime: 1.2, turn: 3.0 },
  radar:       { key: 'radar',       name: 'Radar Missile',  type: 'missile',     dmg: 700, rof: 0.4, speed: 800,  clip: 2,    reload: 7, spread: 0,     splash: 28, lockTime: 2.2, turn: 2.4, needsSensor: true },
  lockmissile: { key: 'lockmissile', name: 'Lock-On Missile',type: 'lockmissile', dmg: 700, rof: 0.7, speed: 720,  clip: 4,    reload: 5, spread: 0,     splash: 20, lockTime: 3.0, turn: 4.0 },
  bomb:        { key: 'bomb',        name: 'Iron Bomb',      type: 'bomb',        dmg: 1260,rof: 0.5, speed: 0,    clip: 2,    reload: 3.75,spread: 0,    splash: 125 },
  bombheavy:   { key: 'bombheavy',   name: 'Heavy Bomb',     type: 'bomb',        dmg: 3080,rof: 0.3, speed: 0,    clip: 1,    reload: 6, spread: 0,     splash: 225 },
  ...
  aamheavy:    { key: 'aamheavy',    name: 'Heavy AAM',      type: 'missile',     dmg: 850, rof: 0.4, speed: 940,  clip: 2,    reload: 7, spread: 0,     splash: 32, lockTime: 1.8, turn: 2.6 },
  clusterbomb: { key: 'clusterbomb', name: 'Cluster Bomb',   type: 'bomb',        dmg: 1960,rof: 0.4, speed: 0,    clip: 2,    reload: 4.5, spread: 0,   splash: 195 },
  ...
  sraam:       { key: 'sraam',       name: 'Short-Range AAM',type: 'missile',     dmg: 420, rof: 0.6, speed: 700,  clip: 2,    reload: 5, spread: 0,     splash: 22, lockTime: 0.9, turn: 4.2 },
  lraam:       { key: 'lraam',       name: 'Long-Range AAM', type: 'radar',       dmg: 950, rof: 0.3, speed: 1000, clip: 2,    reload: 8, spread: 0,     splash: 30, lockTime: 2.6, turn: 2.2, needsSensor: true },
  antiship:    { key: 'antiship',    name: 'Anti-Ship Missile',type: 'lockmissile',dmg:1900, rof: 0.4, speed: 600,  clip: 2,    reload: 8, spread: 0,     splash: 60, lockTime: 3.0, turn: 1.6 },
  guidedbomb:  { key: 'guidedbomb',  name: 'Guided Bomb',    type: 'bomb',        dmg: 1540,rof: 0.4, speed: 0,    clip: 1,    reload: 4.5, spread: 0,   splash: 138 },
  napalm:      { key: 'napalm',      name: 'Incendiary Bomb',type: 'bomb',        dmg: 980, rof: 0.5, speed: 0,    clip: 2,    reload: 3.75,spread: 0,    splash: 275 },
  ...
  cruise:      { key: 'cruise',      name: 'Cruise Missile', type: 'lockmissile', dmg: 1900,rof: 0.25,speed: 480,  clip: 1,    reload: 10, spread: 0,     splash: 70, lockTime: 3.5, turn: 1.0 },
  ...
  navalcannon:  { key: 'navalcannon',  name: 'Naval Cannon',        type: 'gun', dmg: 150, rof: 1.5, speed: 950,  clip: 90,   reload: 6,  spread: 0.004,  splash: 36, heatPerShot: 900,  tracer: '#ffd24d' },
...
function buildNavalTurret(T, d, o = {}){
  ...
  // SOLID WIDE BASE — a deck ring + barbette nearly as broad as the gun house, so the
  // turret sits planted on the deck instead of balancing on a narrow cone (top-heavy).
  const ring = cyl(T, W * 0.66, W * 0.7, H * 0.12, '#343a40', { metal: 0.78, rough: 0.45 }); ring.position.y = -H * 0.3; g.add(ring);
  const barb = cyl(T, W * 0.58, W * 0.64, H * 0.3, '#3c434a', { metal: 0.78, rough: 0.4 }); barb.position.y = -H * 0.12; g.add(barb);
  // armoured gun house with a sloped front glacis
  const house = box(T, W * 0.82, H * 0.5, L * 0.5, col, steel); house.position.set(0, H * 0.14, -L * 0.04); g.add(house);
  const glac = box(T, W * 0.82, H * 0.42, L * 0.24, col, steel); glac.position.set(0, H * 0.08, L * 0.2); glac.rotation.x = -0.6; g.add(glac);
  const roof = box(T, W * 0.7, H * 0.07, L * 0.42, '#69727b', { metal: 0.6 }); roof.position.set(0, H * 0.38, -L * 0.04); g.add(roof);
  for (const sx of [-1, 1]){ const ear = box(T, W * 0.07, H * 0.18, L * 0.1, dark, { metal: 0.55 }); ear.position.set(sx * W * 0.38, H * 0.22, -L * 0.12); g.add(ear); }  // rangefinder ears
  const dirc = cyl(T, W * 0.08, W * 0.1, H * 0.14, '#444b52', { metal: 0.6 }); dirc.position.set(0, H * 0.44, -L * 0.12); g.add(dirc);   // roof director
  // BARRELS (+Z) — long, slender naval rifles with a recoil sleeve and a slight muzzle
  // taper. Spacing widens with calibre so the twin/triple don't fuse into one slab.
  const off = barrels === 1 ? [0] : barrels === 2 ? [-0.26, 0.26] : [-0.32, 0, 0.32];
  const bR = (barrels >= 3 ? 0.1 : barrels === 2 ? 0.11 : 0.13) * W;
  const by = H * 0.06;
  for (const ox of off){ const x = ox * W;
    const sleeve = cylZ(T, bR * 1.45, bR * 1.45, L * 0.16, '#3a4046', { metal: 0.7 }); sleeve.position.set(x, by, L * 0.28); g.add(sleeve);
    const bar = cylZ(T, bR, bR * 0.8, L * 0.8, '#1c1f22', { metal: 0.9, rough: 0.3 }); bar.position.set(x, by, L * 0.66); g.add(bar);
    const mz = cylZ(T, bR * 1.18, bR * 1.18, L * 0.05, '#0a0a0b'); mz.position.set(x, by, L * 1.02); g.add(mz); }
  return g; }
