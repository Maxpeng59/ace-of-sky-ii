...
function installInput(){
  ...
  const onDown = (e) => {
    ...
    if (e.button === 0){
      ...
      // left: Shift+drag turns the view (orbit); a plain click places/selects, and a
      // plain (no-Shift) drag does nothing — so you never spin the view by accident.
      S.dragMode = e.shiftKey ? 'orbit' : 'select';
  ...
  const onMove = (e) => {
    ...
    if (S.dragMode === 'orbit' && S.moved){
      S.camYaw -= dx * 0.006;          // Shift+drag: turn the airframe around (yaw + pitch)
      S.camPitch += dy * 0.006;
      updateCamera();
  ...
  const onUp = (e) => {
    ...
    if (e.button === 0 && mode === 'select' && !S.moved){
      handleLeftClick(e);
